...
class Datatable {
    ...
    public function set_database($db_name) {
        $db_data = $this->app->load->database($db_name, TRUE);
        $this->app->db = $db_data;
    }
    ...
    public function select($columns, $backtick_protect = FALSE) {
        ...
        $this->app->db->select($columns, $backtick_protect);
        return $this;
    }
    ...
    public function join($table, $fk, $type = NULL) {
        ...
        $this->app->db->join($table, $fk, $type);
        return $this;
    }
    ...
    public function where($key_condition, $val = NULL, $backtick_protect = TRUE) {
        ...
        $this->app->db->where($key_condition, $val, $backtick_protect);
        return $this;
    }
    ...
    public function or_where($key_condition, $val = NULL, $backtick_protect = TRUE) {
        ...
        $this->app->db->or_where($key_condition, $val, $backtick_protect);
        return $this;
    }
    ...
    public function where_in($key_condition, $val = NULL, $backtick_protect = TRUE) {
        ...
        $this->app->db->where_in($key_condition, $val, $backtick_protect);
        return $this;
    }
    ...
    public function like($key_condition, $val = NULL, $backtick_protect = TRUE) {
        ...
        $this->app->db->like($key_condition, $val, $backtick_protect);
        return $this;
    }
    ...
    private function get_paging() {
        $iStart = $this->app->input->get('start');
        $iLength = $this->app->input->get('length');

        if ($iLength != '' && $iLength != '-1')
            $this->app->db->limit($iLength, ($iStart) ? $iStart : 0);
    }
    ...
    private function get_ordering() {
        ...
        $Data = $this->app->input->get('columns');


        if ($this->app->input->get('order'))
            foreach ($this->app->input->get('order') as $key)
                if ($this->check_cType())
                    $this->app->db->order_by($Data[$key['column']]['data'], $key['dir']);
                else
                    $this->app->db->order_by($this->columns[$key['column']], $key['dir']);
    }
    ...
    private function get_filtering() {
        $filters = $this->app->input->get('columns');
        $sWhere = '';
        foreach ($filters as $filter) {
            if ($filter['search']['value'] != '') {
                if ($filter['name'] != '') 
                {
                    if (is_numeric($filter['search']['value'])) 
                    {
                        $sWhere .= $filter['name'] . " = " . $this->app->db->escape_like_str($filter['search']['value']) . " AND ";
                    }
                    else 
                    {
                        $sWhere .= $filter['name'] . " LIKE '%" . $this->app->db->escape_like_str($filter['search']['value']) . "%' AND ";
                    }
                } 
                else 
                {
                    if (is_numeric($filter['search']['value'])) 
                    {
                        $sWhere .= $filter['data'] . " = " . $this->app->db->escape_like_str($filter['search']['value']) . " AND ";
                    }
                    else 
                    {
                        $sWhere .= $filter['data'] . " LIKE '%" . $this->app->db->escape_like_str($filter['search']['value']) . "%' AND ";
                    }
                }
            }
        }
    ...
    private function get_filtering() {
        ...
        $mColArray = $this->app->input->get('columns');

        $sWhere = '';
        $search = $this->app->input->get('search');
        $sSearch = $this->app->db->escape_like_str(trim($search['value']));
        ...
        if ($sWhere != '')
            $this->app->db->where('(' . $sWhere . ')');

        // TODO : sRangeSeparator

        foreach ($this->filter as $val)
            $this->app->db->where($val[0], $val[1], $val[2]);
    }*/
    ...
    private function get_display_result() {
        return $this->app->db->get($this->table);
    }
    ...
    private function produce_output($output, $charset) {
        ...
        if ($output == 'json') {
            $sOutput = array
                (
                'draw' => intval($this->app->input->get('draw')),
                'recordsTotal' => $iTotal,
    ...
    private function get_total_results() {
        $query = $this->app->db->query('SELECT FOUND_ROWS() AS total_row')->row_array();
    ...
    private function check_cType() {
        $column = $this->app->input->get('columns');
